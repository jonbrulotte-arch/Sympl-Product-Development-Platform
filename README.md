...
- **Bulk ownership transfer** — Admin → Transfer Ownership reassigns projects between managers in bulk: filter by current owner, select from the master list, and hand the set over, optionally keeping the outgoing owner on as an editing member.
...
**Admin → Transfer Ownership** (requires `admin:users`) reassigns projects in bulk — for a manager leaving, changing teams, or handing off a portfolio.
